"""Falling Wedge scanner for the Wedge Family lane.
...
both boundaries slope downward and converge; the primary chapter lane is an
upward breakout through the upper boundary.
"""
...
PATTERN_KEY = "wedges_falling"
DEFAULT_OUT_DIR = Path("artifacts/scanner_v2/falling_wedges_db_source_parity")


@dataclass(frozen=True)
class FallingWedgeConfig:
    width_min_bars: int = 20
    width_max_bars: int = 110
    high_fall_min_pct: float = 4.0
    low_fall_min_pct: float = 3.0
    upper_slope_max_neg_deg: float = -1.5
    lower_slope_max_neg_deg: float = -0.5
    lower_slope_min_neg_deg: float = -16.0
    upper_faster_than_lower_min_deg: float = 0.5
    ...
    @classmethod
    def from_mapping(cls, value: Optional[Mapping[str, Any]] = None) -> "FallingWedgeConfig":
        if value is None:
            return cls()
...
class FallingWedgeDetector:
    def __init__(self, config: Optional[FallingWedgeConfig | Mapping[str, Any]] = None) -> None:
        self.config = config if isinstance(config, FallingWedgeConfig) else FallingWedgeConfig.from_mapping(config)

    def _breakout(self, df: pd.DataFrame, *, start_idx: int, upper: Trendline) -> Tuple[Optional[int], Optional[float], Optional[float], Optional[float]]:
        for idx in range(start_idx, min(len(df), start_idx + self.config.breakout_search_bars)):
            close = _safe_float(df.iloc[idx].get("close"))
            if close is None:
                continue
            upper_level = upper.value_at(idx)
            if close > upper_level * (1.0 + self.config.breakout_threshold):
                volume_ratio = _safe_float(df.iloc[idx].get("volume_ratio"))
                return idx, close, volume_ratio, (close - upper_level) / upper_level * 100.0
    ...
    def scan_window(self, df: pd.DataFrame, pivots: Sequence[Pivot]) -> Optional[Dict[str, Any]]:
        ...
        high_fall_pct = (h1_price - h2_price) / h1_price * 100.0
        low_fall_pct = (l1_price - l2_price) / l1_price * 100.0
        if high_fall_pct < self.config.high_fall_min_pct or low_fall_pct < self.config.low_fall_min_pct:
            return None
        ...
        if upper_deg > self.config.upper_slope_max_neg_deg:
            return None
        if lower_deg > self.config.lower_slope_max_neg_deg or lower_deg < self.config.lower_slope_min_neg_deg:
            return None
        if abs(upper_deg) - abs(lower_deg) < self.config.upper_faster_than_lower_min_deg:
            return None
        ...
        breakout_idx, breakout_price, volume_ratio, clearance_pct = self._breakout(df, start_idx=formation_end + 1, upper=upper)
        ...
        quality_score += min(6.0, max(0.0, abs(upper_deg) - abs(lower_deg)))
        ...
        return {
            ...
            "high_spread_pct": round(float(high_fall_pct), 2),
            "low_rise_pct": round(float(low_fall_pct), 2),
            "low_spread_pct": round(float(low_fall_pct), 2),
            "high_fall_pct": round(float(high_fall_pct), 2),
...
def scan_symbol(
    ...
    *,
    ...
    detector_config: Optional[FallingWedgeConfig | Mapping[str, Any]] = None,
) -> Tuple[list[dict[str, Any]], dict[str, Any]]:
    if len(df_raw) < 160:
        return [], {"rows": int(len(df_raw)), "skipped": "too_few_rows"}
    config = detector_config if isinstance(detector_config, FallingWedgeConfig) else FallingWedgeConfig.from_mapping(detector_config)
...
def _assign_publication_quality_tiers(rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        ...
        high_fall = _safe_float(row.get("high_fall_pct"))
        low_fall = _safe_float(row.get("low_rise_pct"))
        ...
        upper_deg = abs(_safe_float(row.get("upper_slope_deg")) or 0.0)
        lower_deg = abs(_safe_float(row.get("lower_slope_deg")) or 0.0)
        reasons: list[str] = []
        ...
        score = 0.0
        score += _score_band(high_fall, good=9.0, weak=4.0, reverse=False, weight=0.18)
        score += _score_band(low_fall, good=7.0, weak=3.0, reverse=False, weight=0.16)
        score += _score_band(compression, good=0.50, weak=0.85, reverse=True, weight=0.18)
        score += _score_band(upper_deg - lower_deg, good=3.0, weak=0.5, reverse=False, weight=0.12)
...
def summarize(scan: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        **_group_stats(rows),
        ...
        "variant_table": {"falling_wedge": _group_stats(rows)},
        ...
        "experiment_note": "Falling Wedge scanner uses two downward converging boundaries and upward close confirmation.",
    }
...
EVENT_FIELDS = [
    ...
    "low_rise_pct", "low_spread_pct", "high_fall_pct", "compression_ratio",
...
def scan_falling_wedges_db(
    *,
    ...
    config = FallingWedgeConfig.from_mapping(detector_config)
    ...
    for i, row in enumerate(detections):
        row["detection_id"] = f"wedges_falling:{i + 1:06d}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Falling Wedge scanner against Market Cache latest.sqlite.")
