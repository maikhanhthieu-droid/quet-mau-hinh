"""Symmetrical Triangle scanner for the Triangle Family lane.
...
PATTERN_KEY = "triangles_symmetrical"
DEFAULT_OUT_DIR = Path("artifacts/scanner_v2/symmetrical_triangles_db_source_parity")


@dataclass(frozen=True)
class SymmetricalTriangleConfig:
    ...
    @classmethod
    def from_mapping(cls, value: Optional[Mapping[str, Any]] = None) -> "SymmetricalTriangleConfig":
        if value is None:
            return cls()
...
def _evaluate_detection(df: pd.DataFrame, detection: Mapping[str, Any], *, lookahead: int = 120) -> Dict[str, Any]:
    ...
    target_hit = bool(float(future["high"].max()) >= target) if direction == 1 else bool(float(future["low"].min()) <= target)
...
class SymmetricalTriangleDetector:
    def __init__(self, config: Optional[SymmetricalTriangleConfig | Mapping[str, Any]] = None) -> None:
        self.config = config if isinstance(config, SymmetricalTriangleConfig) else SymmetricalTriangleConfig.from_mapping(config)

    def _breakout(self, df: pd.DataFrame, *, start_idx: int, upper: Trendline, lower: Trendline) -> Tuple[Optional[int], Optional[float], Optional[float], Optional[str], Optional[float]]:
        for idx in range(start_idx, min(len(df), start_idx + self.config.breakout_search_bars)):
            ...
            if close > upper_level * (1.0 + self.config.breakout_threshold):
                ...
                return idx, close, volume_ratio, "up", (close - upper_level) / upper_level * 100.0
            if close < lower_level * (1.0 - self.config.breakout_threshold):
                volume_ratio = _safe_float(df.iloc[idx].get("volume_ratio"))
                return idx, close, volume_ratio, "down", (lower_level - close) / lower_level * 100.0
    ...
    def scan_window(self, df: pd.DataFrame, pivots: Sequence[Pivot]) -> Optional[Dict[str, Any]]:
        ...
        if high_fall_pct < self.config.high_fall_min_pct or low_rise_pct < self.config.low_rise_min_pct:
            return None
        ...
        if upper_deg > self.config.upper_slope_max_neg_deg or lower_deg < self.config.lower_slope_min_deg:
            return None
        ...
        if compression_ratio > self.config.compression_max_ratio:
            return None
        breakout_idx, breakout_price, volume_ratio, breakout_direction, clearance_pct = self._breakout(df, start_idx=formation_end + 1, upper=upper, lower=lower)
        ...
        quality_score += min(10.0, low_rise_pct)
        ...
        return {
            ...
            "triangle_resistance": round(float(upper.value_at(int(formation_start))), 4),
            "triangle_support": round(float(lower.value_at(int(formation_start))), 4),
...
def scan_symbol(
    ...
    *,
    ...
    detector_config: Optional[SymmetricalTriangleConfig | Mapping[str, Any]] = None,
) -> Tuple[list[dict[str, Any]], dict[str, Any]]:
    if len(df_raw) < 160:
        return [], {"rows": int(len(df_raw)), "skipped": "too_few_rows"}
    config = detector_config if isinstance(detector_config, SymmetricalTriangleConfig) else SymmetricalTriangleConfig.from_mapping(detector_config)
    ...
    detector = SymmetricalTriangleDetector(config)
...
def _assign_publication_quality_tiers(rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        ...
        score = 0.0
        score += _score_band(high_fall, good=8.0, weak=3.0, reverse=False, weight=0.18)
        ...
        if high_fall is not None and high_fall < 5.0:
            reasons.append("weak_falling_resistance")
        ...
        if (
            ...
            and (high_fall is not None and high_fall >= 5.0)
...
def summarize(scan: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        **_group_stats(rows),
        ...
        "variant_table": {"symmetrical_triangle": _group_stats(rows)},
        "quality_table": {
        ...
        "experiment_note": "Symmetrical Triangle scanner uses falling resistance, rising support, compression, and direction-specific close confirmation.",
    }
...
def scan_symmetrical_triangles_db(
    *,
    db_path: Path,
    out_dir: Path,
    ...
    config = SymmetricalTriangleConfig.from_mapping(detector_config)
    ...
    for i, row in enumerate(detections):
        row["detection_id"] = f"triangles_symmetrical:{i + 1:06d}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Symmetrical Triangle scanner against Market Cache latest.sqlite.")
    ...
    paths = scan_symmetrical_triangles_db(
