"""Three Falling Peaks / Three Rising Valleys scanner.

Bulkowski treats these as three-swing reversal/continuation structures, not as
generic triple tops/bottoms.  This module keeps their detector separate from
the legacy digitized engine so publication and tradable work can depend on a
family-specific source contract.
"""
...
THREE_PATTERNS = (THREE_FALLING_PEAKS, THREE_RISING_VALLEYS)
DEFAULT_OUT_DIR = Path("artifacts/scanner_v2/three_peaks_valleys_family")


@dataclass(frozen=True)
class ThreePeaksValleysConfig:
    ...
    peak_valley_progress_min_pct: float = 1.0
    max_spacing_imbalance: float = 2.8
    breakout_cooldown_bars: int = 55
    max_events_per_symbol: int = 14

    @classmethod
    def from_mapping(cls, value: Optional[Mapping[str, Any]] = None) -> "ThreePeaksValleysConfig":
        if value is None:
            return cls()
...
class ThreePeaksValleysDetector:
    def __init__(self, pattern_key: str, config: Optional[ThreePeaksValleysConfig | Mapping[str, Any]] = None) -> None:
        if pattern_key not in THREE_PATTERNS:
            raise ValueError(f"unsupported three peaks/valleys pattern {pattern_key}")
        self.pattern_key = pattern_key
        self.config = config if isinstance(config, ThreePeaksValleysConfig) else ThreePeaksValleysConfig.from_mapping(config)

    @property
    def direction(self) -> int:
        return -1 if self.pattern_key == THREE_FALLING_PEAKS else 1

    @property
    def expected_sequence(self) -> tuple[PivotType, ...]:
        if self.pattern_key == THREE_FALLING_PEAKS:
    ...
    def scan_window(self, df: pd.DataFrame, pivots: Sequence[Pivot]) -> Optional[Dict[str, Any]]:
        ...
        if prior is None:
            return None
        if self.pattern_key == THREE_FALLING_PEAKS and prior < self.config.prior_trend_min_pct:
            return None
        if self.pattern_key == THREE_RISING_VALLEYS and prior > -self.config.prior_trend_min_pct:
            return None
        ...
        if self.pattern_key == THREE_FALLING_PEAKS:
            ...
            if not (p2 < p1 * (1.0 - self.config.peak_valley_progress_min_pct / 100.0) and p3 < p2 * (1.0 - self.config.peak_valley_progress_min_pct / 100.0)):
                return None
            boundary = min(lows)
            target = boundary - (high - low)
            progression_pct = (p1 - p3) / p1 * 100.0
        else:
            v1, v2, v3 = lows[0], lows[1], lows[2]
            if not (v2 > v1 * (1.0 + self.config.peak_valley_progress_min_pct / 100.0) and v3 > v2 * (1.0 + self.config.peak_valley_progress_min_pct / 100.0)):
                return None
            boundary = max(highs)
            target = boundary + (high - low)
            progression_pct = (v3 - v1) / v1 * 100.0
        if target <= 0:
            return None
        ...
        score += _score_band(progression_pct, good=9.0, weak=2.5, weight=0.16)
        ...
        return {
            ...
            "peak_valley_progress_pct": round(float(progression_pct), 2),
...
def scan_symbol(
    ...
    *,
    ...
    detector_config: Optional[ThreePeaksValleysConfig | Mapping[str, Any]] = None,
) -> Tuple[list[dict[str, Any]], dict[str, Any]]:
    if len(df_raw) < 170:
        return [], {"rows": int(len(df_raw)), "skipped": "too_few_rows"}
    config = detector_config if isinstance(detector_config, ThreePeaksValleysConfig) else ThreePeaksValleysConfig.from_mapping(detector_config)
    ...
    detector = ThreePeaksValleysDetector(pattern_key, config)
...
def _assign_publication_quality_tiers(rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        ...
        score += _score_band(_safe_float(row.get("peak_valley_progress_pct")), good=9.0, weak=2.5, weight=0.18)
        ...
        if _safe_float(row.get("peak_valley_progress_pct")) is not None and float(row["peak_valley_progress_pct"]) < 5.0:
            reasons.append("weak_step_progression")
...
def _group_stats(rows: Sequence[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "median_step_progress_pct": _median([row.get("peak_valley_progress_pct") for row in rows]),
    }
    ...
    return {
        ...
        **_group_stats(rows),
        ...
        "quantile_metrics": {
            ...
            "step_progress_pct": _quantiles([row.get("peak_valley_progress_pct") for row in rows]),
        },
        "experiment_note": "Three Peaks/Valleys scanner uses five alternating pivots, source-aligned step progression, confirmation beyond the two inner swing boundary, and full-height target as legacy benchmark.",
    }
...
def scan_three_peaks_valleys_db(
    *,
    pattern_key: str,
    db_path: Path,
    out_dir: Path,
    ...
    config = ThreePeaksValleysConfig.from_mapping(detector_config)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Three Peaks/Valleys Family scanners against Market Cache latest.sqlite.")
    parser.add_argument("--pattern", choices=[*THREE_PATTERNS, "all"], default="all")
    ...
    patterns = list(THREE_PATTERNS) if args.pattern == "all" else [args.pattern]
    outputs: dict[str, str] = {}
    for pattern_key in patterns:
        paths = scan_three_peaks_valleys_db(
            pattern_key=pattern_key,
