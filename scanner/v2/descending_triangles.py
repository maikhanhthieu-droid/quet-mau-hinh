"""Descending Triangle scanner for the Triangle Family lane.
...
PATTERN_KEY = "triangles_descending"
DEFAULT_OUT_DIR = Path("artifacts/scanner_v2/descending_triangles_db_source_parity")


@dataclass(frozen=True)
class DescendingTriangleConfig:
    ...
    @classmethod
    def from_mapping(cls, value: Optional[Mapping[str, Any]] = None) -> "DescendingTriangleConfig":
        if value is None:
            return cls()
...
class DescendingTriangleDetector:
    def __init__(self, config: Optional[DescendingTriangleConfig | Mapping[str, Any]] = None) -> None:
        self.config = config if isinstance(config, DescendingTriangleConfig) else DescendingTriangleConfig.from_mapping(config)

    def _breakout(self, df: pd.DataFrame, *, start_idx: int, support: float) -> Tuple[Optional[int], Optional[float], Optional[float]]:
    ...
    def scan_window(self, df: pd.DataFrame, pivots: Sequence[Pivot]) -> Optional[Dict[str, Any]]:
        ...
        support = float(np.median([l1_price, l2_price]))
        if support <= 0 or h1_price <= 0:
            return None
        if h1_price <= support or h2_price <= support:
            return None
        low_spread_pct = abs(l2_price - l1_price) / support * 100.0
        high_fall_pct = (h1_price - h2_price) / h1_price * 100.0
        if low_spread_pct > self.config.low_flat_tol_pct or high_fall_pct < self.config.high_fall_min_pct:
            return None
        upper_deg = _slope_degrees(int(h1.idx), h1_price, int(h2.idx), h2_price)
        lower_deg_abs = abs(_slope_degrees(int(l1.idx), l1_price, int(l2.idx), l2_price))
        if lower_deg_abs > self.config.lower_slope_abs_max_deg or upper_deg > self.config.upper_slope_max_neg_deg:
            return None
        height_abs = max(h1_price, h2_price) - support
        height_pct = height_abs / support * 100.0
        if height_pct < self.config.height_min_pct or height_pct > self.config.height_max_pct:
            return None
        first_gap = h1_price - support
        last_gap = h2_price - support
        ...
        if compression_ratio > self.config.compression_max_ratio:
            return None
        breakout_idx, breakout_price, volume_ratio = self._breakout(df, start_idx=formation_end + 1, support=support)
        ...
        target_price = support - height_abs
        breakout_clearance_pct = (support - float(breakout_price)) / support * 100.0
        quality_score = 62.0
        quality_score += max(0.0, (self.config.low_flat_tol_pct - low_spread_pct) / self.config.low_flat_tol_pct * 13.0)
        ...
        return {
            ...
            "lower_slope_deg": round(float(lower_deg_abs), 2),
            "high_spread_pct": round(float(low_spread_pct), 2),
            "low_rise_pct": round(float(high_fall_pct), 2),
            ...
            "triangle_support": round(float(support), 4),
...
def scan_symbol(
    ...
    *,
    ...
    detector_config: Optional[DescendingTriangleConfig | Mapping[str, Any]] = None,
) -> Tuple[list[dict[str, Any]], dict[str, Any]]:
    if len(df_raw) < 160:
        return [], {"rows": int(len(df_raw)), "skipped": "too_few_rows"}
    config = detector_config if isinstance(detector_config, DescendingTriangleConfig) else DescendingTriangleConfig.from_mapping(detector_config)
...
def _assign_publication_quality_tiers(rows: list[dict[str, Any]]) -> None:
    ...
    for row in rows:
        ...
        if high_spread is not None and high_spread > 1.5:
            reasons.append("support_not_flat_enough")
...
def summarize(scan: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        **_group_stats(rows),
        ...
        "variant_table": {"descending_triangle": _group_stats(rows)},
        "quality_table": {
        ...
        "experiment_note": "Descending Triangle scanner uses flat support, falling resistance, compression, and downward close confirmation.",
    }
...
def scan_descending_triangles_db(
    *,
    db_path: Path,
    out_dir: Path,
    ...
    config = DescendingTriangleConfig.from_mapping(detector_config)
    ...
    for i, row in enumerate(detections):
        row["detection_id"] = f"triangles_descending:{i + 1:06d}"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Descending Triangle scanner against Market Cache latest.sqlite.")
    ...
    paths = scan_descending_triangles_db(
