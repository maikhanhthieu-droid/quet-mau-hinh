"""Build source-grounded Triple Bottoms / Triple Tops public-chapter seed artifacts.
...
from scanner.triple_family_publication_specs import build_triple_patterns_publication_spec  # noqa: E402


DEFAULT_OUT_DIR = Path("artifacts/scanner_v2/triple_family_public_chapters")
...
PATTERNS: dict[str, dict[str, Any]] = {
    "triple_tops": {
        "slug": "triple_tops",
        "title": "Ba đỉnh ngang vùng",
        "subtitle": "Ba đỉnh gần cùng vùng giá trước một phá vỡ xuống",
        "scan_dir": Path("artifacts/scanner_v2/triple_family/triple_tops/db_active"),
        "source_chapter": 51,
        "source_name": "Triple Tops",
        "source_book_pages": [779, 780, 781, 782, 783, 784, 785, 786, 787, 788, 789],
        "source_review_pages": [802, 803, 804, 805, 806, 807, 808, 809, 810, 811, 812],
        ...
        "claim_level": "đọc như ba đỉnh gần cùng vùng giá được xác nhận khi giá đóng cửa xuống dưới đường xác nhận",
        "public_classification_sentence": "Trong phạm vi dữ liệu hiện có, Ba đỉnh ngang vùng nên là chương defensive/informational: mẫu giúp nhận diện áp lực suy yếu theo từng nhịp hồi, nhưng không mặc định là cơ hội short cổ phiếu cơ sở.",
        "morphology": "Ba đỉnh ngang vùng gồm ba đỉnh gần cùng một vùng giá, được ngăn cách bởi hai nhịp lùi tạo đường xác nhận bên dưới. Mẫu chỉ được xác nhận khi giá đóng cửa xuống dưới đường xác nhận đó, cho thấy vùng kháng cự đã chặn được ba lần thử tăng.",
    ...
    "triple_bottoms": {
        "slug": "triple_bottoms",
        "title": "Ba đáy ngang vùng",
        "subtitle": "Ba đáy gần cùng vùng giá trước một phá vỡ lên",
        "scan_dir": Path("artifacts/scanner_v2/triple_family/triple_bottoms/db_active"),
        "source_chapter": 50,
        "source_name": "Triple Bottoms",
        "source_book_pages": [765, 766, 767, 768, 769, 770, 771, 772, 773, 774, 775],
        "source_review_pages": [788, 789, 790, 791, 792, 793, 794, 795, 796, 797, 798],
        "scope_tier": "premium+standard",
        "classification": "hồ sơ đảo chiều tăng/tiếp diễn tăng; có thể kiểm tra tradable layer cho nhánh long-cash",
        "claim_level": "đọc như ba đáy gần cùng vùng giá được xác nhận khi giá đóng cửa vượt đường xác nhận",
        "public_classification_sentence": "Trong phạm vi dữ liệu hiện có, Ba đáy ngang vùng là một chương long-watchlist đáng nghiên cứu: hình thái thể hiện lực bán yếu dần qua ba đáy, nhưng cần đọc cùng kéo ngược vì throwback sau phá vỡ xuất hiện nhiều.",
        "morphology": "Ba đáy ngang vùng gồm ba đáy gần cùng một vùng giá, được ngăn cách bởi hai nhịp hồi tạo đường xác nhận bên trên. Mẫu chỉ có hiệu lực khi giá đóng cửa vượt đường xác nhận đó, xác nhận rằng vùng hỗ trợ đã đứng vững qua ba lần kiểm tra.",
...
def _plot_schematic(out_path: Path, *, pattern_id: str) -> None:
    ...
    if pattern_id == "triple_tops":
        ...
        ys = np.array([19.4, 14.8, 19.0, 14.2, 19.2, 13.2])
        labels = ("đỉnh 1", "đáy xác nhận", "đỉnh 2", "đáy xác nhận", "đỉnh 3", "phá vỡ")
        title = "Giải phẫu Ba đỉnh ngang vùng"
        ax.axhline(float(np.mean([ys[0], ys[2], ys[4]])), color="#8aa6a3", linestyle="--", linewidth=0.9)
        ax.axhline(float(min(ys[1], ys[3])), color="#d28f45", linestyle="--", linewidth=0.9)
    else:
        xs = np.array([0, 1, 2, 3, 4, 5])
        ys = np.array([11.2, 16.0, 11.6, 16.8, 11.4, 18.2])
        labels = ("đáy 1", "đỉnh xác nhận", "đáy 2", "đỉnh xác nhận", "đáy 3", "phá vỡ")
        title = "Giải phẫu Ba đáy ngang vùng"
        ax.axhline(float(np.mean([ys[0], ys[2], ys[4]])), color="#8aa6a3", linestyle="--", linewidth=0.9)
        ax.axhline(float(max(ys[1], ys[3])), color="#d28f45", linestyle="--", linewidth=0.9)
...
def _source_notes(pattern_id: str, meta: Mapping[str, Any]) -> dict[str, Any]:
    is_valleys = pattern_id == "triple_bottoms"
    prefix = "ttb" if is_valleys else "ttop"
    return {
        "status": "PASS",
        ...
        "direct_pdf_review": {
            ...
            "target_rule_summary": "Project the full pattern height from the confirmation boundary in the breakout direction.",
            "review_note": "Đã đối chiếu trực tiếp chương Triple Bottoms / Triple Tops trong PDF gốc trước khi dựng scanner và chapter.",
        },
        "source_rules": [
            {"rule_id": f"{prefix}.three_points", "short_excerpt": "triple patterns", "implementation_mapping": "mẫu phải có ba điểm chính liên tiếp, không phải hai điểm hoặc một vùng dao động chung"},
            {"rule_id": f"{prefix}.near_level_extremes", "short_excerpt": "tops/bottoms near the same price area", "implementation_mapping": "ba đỉnh hoặc ba đáy phải nằm gần cùng một vùng giá; nếu ba điểm trôi xa nhau thì không còn là Triple đúng nghĩa"},
...
def _spec(pattern_id: str, meta: Mapping[str, Any]) -> dict[str, Any]:
    ...
    confirm = "vượt đường xác nhận nối vùng đỉnh xen giữa" if is_valleys else "rơi xuống dưới đường xác nhận nối vùng đáy xen giữa"
    near_level = "gần cùng vùng giá"
    return {
        "title": meta["title"],
        ...
        "schematic_caption": f"Sơ đồ minh họa {meta['title']}: ba {point_name} gần cùng vùng giá, hai điểm xen giữa và đường xác nhận.",
        "how_subtitle": "Ba điểm chính trước, xác nhận sau.",
        "labels": {"favorable_move": "mức đi thuận lợi", "adverse_move": "mức đi ngược bất lợi"},
        "source_rule_ids": ["three_points", "near_level_extremes", "proportional", "confirmation", "height_target"],
        "public_rule_rows": [
            [f"Phải có ba {point_name} liên tiếp.", f"Ba {point_name} là thân mẫu; thiếu một điểm thì không gọi là {meta['title']}."],
            [f"Ba {point_name} phải {near_level}.", "Bộ nhận diện yêu cầu ba điểm chính nằm gần một vùng hỗ trợ/kháng cự, tránh nhầm với mô hình có đỉnh hoặc đáy trôi xa."],
        ...
        "quick_question_rows": [
            ["Hình thái", f"Có đúng ba {point_name} gần cùng vùng giá không?"],
        ...
        "component_rows": [
            ...
            [f"{point_name.capitalize()} thứ hai", "Điểm thứ hai phải trở lại gần vùng giá cũ.", "Cho thấy vùng hỗ trợ/kháng cự còn hiệu lực"],
            [f"{point_name.capitalize()} thứ ba", "Điểm thứ ba kiểm tra lại vùng giá đó.", "Không được quá lệch"],
            ["Vùng xác nhận", "Giá đóng cửa qua vùng hai điểm xen giữa.", "Không đo trước xác nhận"],
        ],
        "reject_bullets": [
            f"Không có đủ ba {point_name} rõ ràng.",
            f"{point_name.capitalize()} thứ ba lệch quá xa khỏi vùng giá chung.",
        ...
        "identification_bridge": (
            f"Các quy tắc nhận diện nên được đọc từ ba {point_name} gần cùng vùng giá, tới đường xác nhận, rồi mới tới kết quả sau phá vỡ. "
        ...
        "quantile_specs": [
            ...
            ("Độ lệch ba điểm", "extreme_spread_pct", "%"),
            ("Độ rộng nhịp xen giữa", "inner_swing_pct", "%"),
        ...
        "skip_condition_specs": [
            ...
            ("Ba điểm không ngang vùng", "extreme_spread_pct", "q75", None, "Ba điểm chính lệch nhau quá rộng nên mẫu mất chất Triple."),
            ("Nhịp xen giữa quá hẹp", "inner_swing_pct", "q25_abs", None, "Hai nhịp xen giữa quá nhỏ làm đường xác nhận thiếu ý nghĩa."),
        ...
        "general_stat_specs": [
            ...
            ("Độ lệch ba điểm", "extreme_spread_pct", "%", "Cho biết ba điểm chính có thật sự nằm gần cùng một vùng giá hay không."),
            ("Độ rộng nhịp xen giữa", "inner_swing_pct", "%", "Cho biết đường xác nhận có ý nghĩa hay chỉ là dao động quá hẹp."),
...
def _publication_payload(pattern_id: str, meta: Mapping[str, Any], events: pd.DataFrame, all_events: pd.DataFrame, path_df: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "chapter_reference": {
            ...
            "median_extreme_spread_pct": _fmt(pd.to_numeric(events.get("extreme_spread_pct"), errors="coerce").median()),
            "median_inner_swing_pct": _fmt(pd.to_numeric(events.get("inner_swing_pct"), errors="coerce").median()),
...
def build_one_triple_patterns_chapter(*, pattern_id: str, out_dir: Path) -> dict[str, Path]:
    ...
    publication_spec = build_triple_patterns_publication_spec(pattern_id=pattern_id, title=str(meta["title"]), spec=spec)
    ...
    entry = {
        ...
        "pdf": f"artifacts/final_chapters/triple_family/{meta['slug']}_final.pdf",
        "source_pdf": f"artifacts/final_chapters/triple_family/{meta['slug']}_final.pdf",
        ...
        "note": "Triple Patterns Family dùng scanner riêng; builder này chỉ cung cấp nguyên liệu, không render hoặc approve PDF final.",
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build Triple Bottoms / Triple Tops public-chapter seed artifacts.")
    ...
    for pattern_id in patterns:
        outputs[pattern_id] = {key: str(value) for key, value in build_one_triple_patterns_chapter(pattern_id=pattern_id, out_dir=Path(args.out_dir)).items()}
