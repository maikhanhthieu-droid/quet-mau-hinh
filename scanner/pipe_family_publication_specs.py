"""Publication-semantic specs for Pipe Family chapters."""
...
PIPE_FAMILY_PUBLICATION_SPEC_VERSION = "pipe_family_publication_spec_v1"
...
PATTERN_REQUIRED_PHRASES = {
    "pipe_bottoms": (
        "Pipe Bottoms",
        "hai cú xuyên giá giảm liền nhau",
        "biểu đồ tuần",
        "đóng cửa vượt đỉnh cao nhất của pipe",
    ),
    "pipe_tops": (
        "Pipe Tops",
        "hai cú xuyên giá tăng liền nhau",
        "biểu đồ tuần",
        "đóng cửa xuống dưới đáy thấp nhất của pipe",
    ),
}


def build_pipe_publication_spec(*, pattern_id: str, title: str, spec: Mapping[str, Any]) -> dict[str, Any]:
    publication_spec = dict(spec)
    publication_spec.update({
        "status": "PASS",
        "semantic_gate_id": PUBLICATION_SEMANTIC_GATE_ID,
        "publication_spec_id": f"{pattern_id}_{PIPE_FAMILY_PUBLICATION_SPEC_VERSION}",
