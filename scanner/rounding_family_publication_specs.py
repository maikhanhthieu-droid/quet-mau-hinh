"""Publication-semantic specs for Rounding Family chapters."""
...
ROUNDING_FAMILY_PUBLICATION_SPEC_VERSION = "rounding_family_publication_spec_v1"
...
PATTERN_REQUIRED_PHRASES = {
    "rounding_bottoms": (
        ...
        "dạng bát",
        "biểu đồ tuần",
        "đóng cửa vượt mép phải",
    ),
    "rounding_tops": (
        "Rounding Tops",
        "dạng bát úp",
        "biểu đồ tuần",
        "đóng cửa xuống dưới mép phải",
    ),
}


def build_rounding_publication_spec(*, pattern_id: str, title: str, spec: Mapping[str, Any]) -> dict[str, Any]:
    publication_spec = dict(spec)
    publication_spec.update({
        "status": "PASS",
        "semantic_gate_id": PUBLICATION_SEMANTIC_GATE_ID,
        "publication_spec_id": f"{pattern_id}_{ROUNDING_FAMILY_PUBLICATION_SPEC_VERSION}",
