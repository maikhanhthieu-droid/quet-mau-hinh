"""Build source-grounded Pipe Family public-chapter seed artifacts.
...
from scanner.pipe_family_publication_specs import build_pipe_publication_spec  # noqa: E402
...
DEFAULT_OUT_DIR = Path("artifacts/scanner_v2/pipe_family_public_chapters")
...
PATTERNS: dict[str, dict[str, Any]] = {
    "pipe_bottoms": {
        "slug": "pipe_bottoms",
        "title": "Pipe Bottoms",
        "subtitle": "Hai cú xuyên giá giảm liền nhau trên biểu đồ tuần",
        "scan_dir": Path("artifacts/scanner_v2/pipe_family/pipe_bottoms/db_active"),
        "source_chapter": 35,
        "source_name": "Pipe Bottoms",
        "source_book_pages": [536, 537, 538, 539, 540, 541, 542, 543, 544, 545, 546, 547, 548, 549],
        "source_review_pages": [559, 560, 561, 562, 563, 564, 565, 566, 567, 568, 569, 570, 571, 572],
        ...
        "claim_level": "đọc như hai cú xuyên giá giảm liền nhau được xác nhận khi giá đóng cửa vượt đỉnh pipe",
        "public_classification_sentence": "Trong phạm vi dữ liệu hiện có, Pipe Bottoms là chương long-watchlist đáng nghiên cứu: hình thái weekly rõ, mẫu đủ dày, nhưng vẫn cần đọc cùng kéo ngược và thời gian xác nhận vì throwback/retest xuất hiện nhiều.",
        "morphology": "Pipe Bottoms gồm hai cú xuyên giá giảm liền nhau trên biểu đồ tuần. Hai đáy phải gần bằng nhau, hai tuần phải có vùng giá chồng lấn rõ, mỗi cú xuyên phải nổi bật so với vùng giá xung quanh, và mẫu chỉ được xác nhận khi giá đóng cửa vượt lên trên đỉnh cao nhất của cặp pipe.",
    ...
    "pipe_tops": {
        "slug": "pipe_tops",
        "title": "Pipe Tops",
        "subtitle": "Hai cú xuyên giá tăng liền nhau trên biểu đồ tuần",
        "scan_dir": Path("artifacts/scanner_v2/pipe_family/pipe_tops/db_active"),
        "source_chapter": 36,
        "source_name": "Pipe Tops",
        "source_book_pages": [550, 551, 552, 553, 554, 555, 556, 557, 558, 559, 560, 561],
        "source_review_pages": [573, 574, 575, 576, 577, 578, 579, 580, 581, 582, 583, 584],
        "scope_tier": "premium+standard",
        "classification": "hồ sơ phòng thủ/thoát vị thế vì nhánh chính là cảnh báo giảm trên cổ phiếu cơ sở",
        "claim_level": "đọc như hai cú xuyên giá tăng liền nhau được xác nhận khi giá đóng cửa xuống dưới đáy pipe",
        "public_classification_sentence": "Trong phạm vi dữ liệu hiện có, Pipe Tops nên là chương defensive/informational: mẫu giúp nhận diện vùng đỉnh ngắn hạn hoặc nhịp hồi trong xu hướng giảm, nhưng không mặc định là cơ hội short cổ phiếu cơ sở.",
        "morphology": "Pipe Tops gồm hai cú xuyên giá tăng liền nhau trên biểu đồ tuần. Hai đỉnh phải gần bằng nhau, hai tuần phải có vùng giá chồng lấn rõ, mỗi cú xuyên phải nổi bật so với vùng giá xung quanh, và mẫu chỉ được xác nhận khi giá đóng cửa xuống dưới đáy thấp nhất của cặp pipe.",
...
def _plot_schematic(out_path: Path, *, pattern_id: str) -> None:
    ...
    if pattern_id == "pipe_bottoms":
        ...
        ax.text(0.1, max(highs[1], highs[3]) + 0.2, "xác nhận: đóng cửa vượt đỉnh pipe", fontsize=8, color="#7A5195")
    else:
        ax.axhline(min(lows[1], lows[3]), color="#7A5195", linestyle="--", linewidth=1.0)
        ax.text(0.1, min(lows[1], lows[3]) - 0.8, "xác nhận: đóng cửa xuống dưới đáy pipe", fontsize=8, color="#7A5195")
...
def _source_notes(pattern_id: str, meta: Mapping[str, Any]) -> dict[str, Any]:
    is_bottom = pattern_id == "pipe_bottoms"
    prefix = "pb" if is_bottom else "pt"
    ...
    confirm_vi = "vượt đỉnh cao nhất của cặp pipe" if is_bottom else "rơi xuống dưới đáy thấp nhất của cặp pipe"
    return {
        "status": "PASS",
        ...
        "direct_pdf_review": {
            ...
            "target_rule_summary": "Measure the pipe height and project it from the confirmation price in breakout direction.",
            "review_note": "Đã đối chiếu trực tiếp chương Pipe trong PDF gốc trước khi dựng scanner và chapter.",
        },
        "source_rules": [
            {"rule_id": f"{prefix}.weekly_chart", "short_excerpt": "Use the weekly chart", "implementation_mapping": "chỉ đọc trên nến tuần; tín hiệu ngày không đủ để gọi là Pipe"},
            {"rule_id": f"{prefix}.two_adjacent_spikes", "short_excerpt": f"two adjacent {spike} price spikes", "implementation_mapping": f"hai cú xuyên giá {spike_vi} trên hai tuần liền nhau"},
            ...
            {"rule_id": f"{prefix}.large_overlap", "short_excerpt": "large price overlap", "implementation_mapping": "hai tuần phải có vùng giá chồng lấn lớn, không phải một tuần lớn ghép với một tuần nhỏ"},
            ...
            {"rule_id": f"{prefix}.obvious_pipe", "short_excerpt": "stick out like a sore thumb", "implementation_mapping": "mẫu phải nhìn thấy ngay trên biểu đồ tuần; nếu phải gượng ép thì nên bỏ qua"},
            ...
            {"rule_id": f"{prefix}.height_target", "short_excerpt": "measure the height of the pipe", "implementation_mapping": "mục tiêu được đo từ chiều cao của cặp pipe; 0,5x là mốc đọc thận trọng, 1,0x là mốc đầy đủ"},
        ],
    }
...
def _spec(pattern_id: str, meta: Mapping[str, Any]) -> dict[str, Any]:
    ...
    confirm_rule = "đóng cửa tuần vượt đỉnh cao nhất của cặp pipe" if is_bottom else "đóng cửa tuần rơi xuống dưới đáy thấp nhất của cặp pipe"
    false_pair = "một tuần giảm mạnh ghép với một tuần giảm yếu" if is_bottom else "một tuần tăng mạnh ghép với một tuần tăng yếu"
    quick_question_rows = (
        [
            ["Khung thời gian", "Pipe đáy đã được đọc trên hai nến tuần liền nhau chưa?"],
            ["Hình thái đáy", "Hai cú xuyên xuống có nổi bật, gần bằng nhau và chồng lấn rõ không?"],
            ["Xác nhận tăng", "Giá đã đóng cửa tuần vượt đỉnh cao nhất của cặp pipe đáy chưa?"],
            ["Đường đi sau xác nhận", "Mốc 0,5x chiều cao pipe đáy có đến trước kéo ngược 5% không?"],
        ]
        if is_bottom
        else [
            ["Khung thời gian", "Pipe đỉnh đã được đọc trên hai nến tuần liền nhau chưa?"],
            ["Hình thái đỉnh", "Hai cú xuyên lên có nổi bật, gần bằng nhau và chồng lấn rõ không?"],
            ["Xác nhận giảm", "Giá đã đóng cửa tuần rơi xuống dưới đáy thấp nhất của cặp pipe đỉnh chưa?"],
            ["Đường đi sau xác nhận", "Mốc 0,5x chiều cao pipe đỉnh có đến trước nhịp hồi ngược 5% không?"],
        ]
    )
    return {
        "title": meta["title"],
        ...
        "target_unit_label": "chiều cao pipe",
        "target_focus_title": "Mốc cơ sở 0,5x",
        "target_focus_caption": "mốc 0,5x chiều cao pipe",
        "target_focus_reading": "mốc thận trọng trước khi đọc mục tiêu đầy đủ",
        "target_full_title": "Mốc nguồn 1,0x",
        "target_full_reading": "mốc đầy đủ theo chiều cao pipe",
        "morphology_sentence": meta["morphology"],
        "role_note": meta["role_note"],
        "classification_sentence": meta["public_classification_sentence"],
        "headline_scope": "Pipe được nhận diện trên biểu đồ tuần; biến động trong ngày hoặc trên nến ngày không đủ để kết luận mẫu.",
        "local_source_chapter": meta["source_chapter"],
        "schematic_caption": f"Sơ đồ minh họa {meta['title']}: hai cú xuyên giá liền nhau, vùng chồng lấn lớn, rồi đóng cửa xác nhận.",
        "how_subtitle": "Hai cú xuyên giá trên nến tuần: hình thái trước, xác nhận sau.",
        "labels": {"favorable_move": "mức đi thuận lợi", "adverse_move": "mức đi ngược bất lợi"},
        "source_rule_ids": ["pipe.weekly_chart", "pipe.two_adjacent_spikes", "pipe.large_overlap", "pipe.confirmation", "pipe.height_target"],
        "public_rule_rows": [
            ["Đọc trên nến tuần.", "Mỗi vạch pipe đại diện cho một tuần giao dịch; tín hiệu ngày chỉ dùng để quan sát thêm."],
            [f"Hai cú xuyên giá {spike_dir} liền nhau.", f"Hai tuần {spike_dir} mạnh phải đứng sát nhau và cùng tạo hình như hai vạch song song."],
            ["Hai vạch có độ dài tương đương.", f"Nếu một vạch quá ngắn, mẫu giống {false_pair} hơn là một Pipe rõ."],
            ["Vùng giá hai tuần chồng lấn lớn.", "Hai tuần phải có nhiều vùng giá chung; pipe không phải một cú sốc đơn lẻ."],
            ["Chỉ tính sau xác nhận.", f"Mẫu có hiệu lực khi giá {confirm_rule}; trước đó chỉ là hình thái đang hình thành."],
            ["Mục tiêu đo bằng chiều cao pipe.", "Mốc 0,5x dùng để đọc thận trọng; mốc 1,0x là mục tiêu đầy đủ theo chiều cao mẫu."],
        ],
        ...
        "component_rows": [
            ...
            ["Cú xuyên thứ hai", "Tuần kế tiếp lặp lại cú xuyên gần cùng vùng giá.", "Gần bằng vạch thứ nhất"],
            ["Vùng chồng lấn", "Hai tuần có phần giá giao nhau đủ lớn để nhìn như một cặp pipe.", "Không ghép một vạch lớn với một vạch nhỏ"],
            ["Xác nhận", "Giá đóng cửa tuần đi qua ranh giới quan trọng của cặp pipe.", "Không kết luận trước nến xác nhận"],
        ],
        "reject_bullets": [
            "Hai cú xuyên giá không liền nhau hoặc cách nhau quá xa.",
            "Một vạch quá nhỏ so với vạch còn lại.",
            "Không có đóng cửa xác nhận sau pipe.",
            "Đường giá xung quanh quá răng cưa khiến pipe không nổi bật.",
        ],
        "identification_paragraphs": [meta["morphology"]],
        "example_intro": ["Ba ví dụ dưới đây được đọc trên nến tuần: một mẫu đạt mốc cơ sở, một mẫu gần trung vị và một mẫu thất bại. Bảng diễn biến đi kèm từng ví dụ để người đọc thấy rõ cú xuyên thứ nhất, cú xuyên thứ hai và tuần xác nhận."],
        "failure_bullets": [
            "Thất bại 5% là mẫu không đi đủ xa sau xác nhận, không phải stop-loss thực chiến.",
            "Pipe có thể kiểm định lại vùng xuyên giá, vì vậy cần đọc cùng mức kéo ngược.",
        ...
        "quick_conclusion_rows": [
            ...
            ["Mốc tham chiếu?", "1,0x chiều cao pipe theo nguồn."],
            ["Khi nào thận trọng?", "Khi xác nhận quá trễ, hai cú xuyên lệch nhau, hoặc vùng chồng lấn yếu."],
        ],
        "identification_bridge": (
            "Các quy tắc nhận diện nên được đọc như một phép kiểm hình học trên nến tuần: trước hết phải thấy hai cú xuyên giá liền nhau, "
            "sau đó kiểm tra chúng có nổi bật và chồng lấn đủ lớn không, cuối cùng mới chờ giá đóng cửa qua ranh giới xác nhận. "
            "Nếu đảo thứ tự này, người đọc rất dễ gọi một cú biến động đơn lẻ là Pipe."
        ),
        "caveat_bullets": [
        ...
        "quantile_specs": [
            ...
            ("Chiều cao pipe", "pattern_height_pct", "%"),
            ("Độ lệch hai cú xuyên", "spike_similarity_pct", "%"),
            ("Độ chồng lấn", "spike_overlap_pct", "%"),
        ...
        "skip_condition_specs": [
            ("Xác nhận quá trễ", "breakout_lag_bars", "q75", None, "Pipe mất độ sắc nếu giá chờ quá lâu mới xác nhận."),
            ("Hai cú xuyên lệch nhau", "spike_similarity_pct", "q75", None, "Hai cú xuyên không còn giống một pipe rõ ràng."),
            ("Vùng chồng lấn yếu", "spike_overlap_pct", "q25_abs", None, "Hai tuần không có vùng giá chung đủ lớn."),
            ("Kéo ngược quá sâu", "mae_pct", "q75", None, "Đường đi sau xác nhận không còn gọn."),
        ],
        "general_stat_specs": [
            ("Độ dài mẫu", "pattern_width_bars", "tuần", "Pipe là mẫu ngắn, nhưng xác nhận có thể đến sau vài tuần."),
            ("Chiều cao pipe", "pattern_height_pct", "%", "Chiều cao là thước đo target."),
            ("Độ lệch hai cú xuyên", "spike_similarity_pct", "%", "Càng nhỏ càng giống pipe nguồn."),
            ("Độ chồng lấn", "spike_overlap_pct", "%", "Vùng chồng lấn lớn làm hai cú xuyên đọc rõ hơn."),
        ],
        "best_condition_specs": [
            ("Nhóm hình thái tốt", "publication_quality_tier", "==", "premium", "Hai cú xuyên gần bằng nhau, vùng chồng lấn rõ và đường giá sạch."),
        ...
        "conclusion_bullets": [
            f"{meta['title']} chỉ được đọc trên biểu đồ tuần, đúng với hình thái Pipe trong tài liệu nguồn.",
            "Mục tiêu nguồn là 1,0x chiều cao pipe; chương dùng 0,5x làm mốc cơ sở thận trọng.",
            meta["role_note"],
        ],
    }
...
def _publication_payload(pattern_id: str, meta: Mapping[str, Any], events: pd.DataFrame, all_events: pd.DataFrame, path_df: pd.DataFrame) -> dict[str, Any]:
    base = _metric_for_target(events, path_df, 0.5, "conservative_half_pipe")
    full = _metric_for_target(events, path_df, 1.0, "source_full_pipe")
    return {
        ...
        "chapter_reference": {
            ...
            "median_spike_overlap_pct": _fmt(pd.to_numeric(events.get("spike_overlap_pct"), errors="coerce").median()),
        },
        "target_calibration": {
            "target_family": {"conservative_half_pipe": 0.5, "source_full_pipe": 1.0},
            "selected_base_target_multiple": 0.5,
            "selected_base_target_role": "conservative_half_pipe",
            "base_target": base,
            "stretch_target": full,
            "legacy_target": full,
            "rows": [base, full],
            "interpretation": "Mốc 0,5x giữ vai trò cơ sở thận trọng; 1,0x giữ vai trò mốc nguồn theo chiều cao pipe.",
        },
...
def build_one_pipe_chapter(*, pattern_id: str, out_dir: Path, price_db: Path) -> dict[str, Path]:
    ...
    publication_spec = build_pipe_publication_spec(pattern_id=pattern_id, title=str(meta["title"]), spec=spec)
    ...
    style_dossier.write_text(
        ...
        f"Chương nguồn: {meta['source_name']} trong Encyclopedia of Chart Patterns. "
            "Dossier giữ thứ tự đọc: biểu đồ tuần, hai cú xuyên giá liền nhau, vùng chồng lấn, đóng cửa xác nhận, thất bại, mục tiêu và cách dùng thận trọng. "
    ...
    entry = {
        ...
        "pdf": f"artifacts/final_chapters/pipe_family/{meta['slug']}_final.pdf",
        "source_pdf": f"artifacts/final_chapters/pipe_family/{meta['slug']}_final.pdf",
        ...
        "note": "Pipe Family dùng scanner weekly riêng; builder này chỉ cung cấp nguyên liệu, không render hoặc approve PDF final.",
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build Pipe Family public-chapter seed artifacts.")
    ...
    for pattern_id in patterns:
        outputs[pattern_id] = {key: str(value) for key, value in build_one_pipe_chapter(pattern_id=pattern_id, out_dir=Path(args.out_dir), price_db=Path(args.price_db)).items()}
