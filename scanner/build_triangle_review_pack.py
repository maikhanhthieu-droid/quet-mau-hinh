...
def _render_markdown(rows: List[Dict[str, Any]], summary: Dict[str, Any], out_dir: Path) -> str:
    lines: List[str] = []
    lines.append("# Triangle Visual Review Pack")
    lines.append("")
    ...
    for row in rows:
        ...
        for key, label in (
            ("sequence_tag", "seq"),
            ("upper_slope_deg", "up_deg"),
            ("lower_slope_deg", "lo_deg"),
            ("compression_ratio", "comp"),
            ("apex_progress_pct", "progress"),
            ("boundary_fit_error_pct", "fit"),
        ):
            val = family.get(key)
            if val is None:
                continue
...
def build_review_pack(
    *,
    ...
    for row in rows:
        symbol = str(row["symbol"])
        df = _load_symbol_ohlcv(str(price_db), symbol)
        if df.empty:
            continue
        ...
        title = (
            f"{row['pattern_name']} | {symbol} | conf={row.get('confidence_score')} | "
