...
def _render_markdown(rows: List[Dict[str, Any]], summary: Dict[str, Any], out_dir: Path) -> str:
    lines: List[str] = []
    lines.append("# Cup With Handle Visual Review Pack")
    lines.append("")
    ...
    for row in rows:
        ...
        for key, label in (
            ("cup_width_bars", "cup_w"),
            ("handle_width_bars", "handle_w"),
            ("bottom_pos_pct", "bottom_pos"),
            ("near_bottom_bars", "bottom_n"),
            ("rim_diff_pct", "rim_diff"),
            ("handle_decline_pct", "handle_drop"),
            ("handle_pos_pct", "handle_pos"),
            ("handle_slope_pct", "handle_slope"),
            ("breakout_lag_bars", "lag"),
        ):
            val = family.get(key)
            if val is None:
                continue
            if isinstance(val, float):
                metrics.append(f"{label}={val:.2f}")
            else:
...
def build_review_pack(
    *,
    ...
    rows = _load_rows(valid_db, split="valid", run_id=valid_run_id)
    rows.extend(_load_rows(calib_db, split="calib", run_id=calib_run_id))
    ...
    _write_text(str(out_dir / "review_gallery.md"), _render_markdown(rendered, summary, out_dir))
