"""Diagnose Double Bottom Adam & Adam tradable blockers."""
...
OUT_DIR = Path("artifacts/scanner_v2/double_bottom_aa_tradable_blocker_audit")
AUDIT_ID = "double_bottom_aa_tradable_blocker_audit_v1"
...
def build_grid() -> list[GenericExecutionConfig]:
    ...
    for target in (0.35, 0.50, 0.65, 0.75):
        for stop in (5.0, 7.0, 10.0):
            for hold in (20, 60):
                for delay in (1, 3):
                    for setup in (None, 65.0, 68.0, 70.0, 72.0, 75.0):
                        for confirm in (None, 60.0, 70.0):
                            configs.append(
                                GenericExecutionConfig(
                                    strategy_id=(
                                        f"double_bottoms_adam_adam__local_t{str(target).replace('.', '')}"
...
def _sample_depth_alternatives(rows: Sequence[Mapping[str, Any]], *, limit: int = 5) -> list[dict[str, Any]]:
    """Surface branches that trade off less purity for enough holdout depth."""
    alternatives = [
        dict(row)
        for row in rows
        if _as_int(row.get("holdout_trades")) >= 12
    ...
    alternatives = sorted(
        alternatives,
        key=lambda row: (
            _as_float(row.get("holdout_total_return_pct")),
            _as_float(row.get("validation_total_return_pct")),
            _as_float(row.get("total_return_pct")),
        ),
        reverse=True,
    )
...
def evaluate_full_candidate(events: pd.DataFrame, path: pd.DataFrame, config: GenericExecutionConfig) -> dict[str, Any]:
    ...
    selection = {
        ...
        "selection_basis": "Double Bottom AA local train+validation shortlist; holdout/walk-forward are promotion evidence",
...
def _guard(best: Mapping[str, Any]) -> dict[str, Any]:
    ...
    checks = [
        {"check": "score_threshold", "status": "fail" if _as_float(best.get("score")) < 95.0 else "pass", "observed": best.get("score"), "rule": "best Double Bottom AA branch must score >= 95"},
        {"check": "promotion_blockers", "status": "fail" if blockers else "pass", "observed": ",".join(sorted(blockers)) or "none", "rule": "best Double Bottom AA branch must have no blocker"},
...
def run_audit(*, out_dir: Path = OUT_DIR, shortlist_size: int = 5) -> dict[str, Path]:
    ...
    payload = {
        ...
        "sample_depth_alternatives": _sample_depth_alternatives(grid_rows),
        "rows": rows,
    }
    paths = {
        "json": out_dir / "double_bottom_aa_tradable_blocker_audit.json",
        "grid": out_dir / "double_bottom_aa_local_strategy_grid.csv",
        "candidates": out_dir / "double_bottom_aa_full_candidate_scores.csv",
        "md": out_dir / "double_bottom_aa_tradable_blocker_audit.md",
    }
...
def render_markdown(payload: Mapping[str, Any]) -> str:
    lines = [
        "# Double Bottom Adam & Adam Tradable Blocker Audit",
        "",
        f"Audit: `{AUDIT_ID}`",
        "",
        ...
        "",
        "## Sample Depth Alternatives",
        "",
        "Các nhánh dưới đây không thay thế nhánh điểm cao nhất. Chúng chỉ cho thấy liệu có cấu hình nào lấy lại cỡ mẫu holdout mà không làm holdout âm.",
        "",
        "| Strategy | Trades | Validation | Holdout | Total | Win | Target exits | Stop exits |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for row in payload.get("sample_depth_alternatives") or []:
        lines.append(
            "| {strategy} | {trades} | {validation} | {holdout} | {total} | {win} | {target} | {stop} |".format(
                ...
                total=row.get("total_return_pct") or "",
                win=row.get("win_rate_pct") or "",
                target=row.get("target_exit_rate_pct") or "",
                stop=row.get("stop_exit_rate_pct") or "",
            )
        )
    lines.extend(
        [
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Double Bottom AA blocker audit.")
