"""Diagnose the Symmetrical Triangle long-up tradable branch.

The public chapter is mixed-direction.  This audit only tests whether the
up-breakout branch can stand as a direct long-cash candidate under the same
entry/exit/cost/sizing/OOS/walk-forward contract.
"""
...
OUT_DIR = Path("artifacts/scanner_v2/symmetrical_triangle_tradable_blocker_audit")
AUDIT_ID = "symmetrical_triangle_long_up_blocker_audit_v1"
...
def _utility(row: Mapping[str, Any], prefix: str = "") -> float:
    ...
    return (
        ...
        + 0.12 * _score_between(row.get(key("trades")), 12.0, 70.0)
...
def build_grid() -> list[GenericExecutionConfig]:
    ...
    for target in (0.50, 0.75, 1.00):
        for stop in (7.0, 10.0):
            for hold in (20, 60):
                for delay in (1, 3):
                    for setup in (None, 70.0):
                        for confirm in (None, 65.0, 75.0):
                            configs.append(
                                GenericExecutionConfig(
                                    strategy_id=(
                                        f"triangles_symmetrical__longup_t{str(target).replace('.', '')}"
    ...
    for target in (0.75, 1.00):
        for regime in (("bull",), ("bear",)):
            configs.append(
                GenericExecutionConfig(
                    strategy_id=f"triangles_symmetrical__longup_ctx_{regime[0]}_t{str(target).replace('.', '')}_q70_c65_liqmh",
                    target_multiple=target,
                    stop_loss_pct=7.0,
                    max_holding_days=60,
                    entry_delay_bars=3,
...
def evaluate_full_candidate(events: pd.DataFrame, path: pd.DataFrame, config: GenericExecutionConfig) -> dict[str, Any]:
    ...
    selection = {
        ...
        "selection_basis": "symmetrical long-up train+validation shortlist; holdout/walk-forward are promotion evidence",
...
def _guard(best: Mapping[str, Any]) -> dict[str, Any]:
    ...
    checks = [
        {"check": "score_threshold", "status": "fail" if _as_float(best.get("score")) < 95.0 else "pass", "observed": best.get("score"), "rule": "best long-up branch must score >= 95"},
        {"check": "promotion_blockers", "status": "fail" if blockers else "pass", "observed": ",".join(sorted(blockers)) or "none", "rule": "best long-up branch must have no blocker"},
...
def run_audit(*, out_dir: Path = OUT_DIR, shortlist_size: int = 5) -> dict[str, Path]:
    ...
    paths = {
        "json": out_dir / "symmetrical_triangle_tradable_blocker_audit.json",
        "grid": out_dir / "symmetrical_triangle_longup_strategy_grid.csv",
        "candidates": out_dir / "symmetrical_triangle_full_candidate_scores.csv",
        "md": out_dir / "symmetrical_triangle_tradable_blocker_audit.md",
    }
...
def render_markdown(payload: Mapping[str, Any]) -> str:
    lines = [
        "# Symmetrical Triangle Long-Up Blocker Audit",
        "",
        f"Audit: `{AUDIT_ID}`",
        "",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Symmetrical Triangle long-up blocker audit.")
