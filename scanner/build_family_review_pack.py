...
def _parse_metric_keys(raw: Optional[str]) -> List[str]:
    if not raw:
        return []
    return [part.strip() for part in str(raw).split(",") if part.strip()]


def _load_rows(results_db_path: Path, *, split: str, run_id: str, patterns: List[str]) -> List[Dict[str, Any]]:
    conn = sqlite3.connect(str(results_db_path))
    conn.row_factory = sqlite3.Row
    try:
        cols = {str(row[1]) for row in conn.execute("PRAGMA table_info(pattern_detections)").fetchall()}
        optional_selects = [
            f"{expr} AS {name}"
...
def _render_markdown(
    ...
    *,
    ...
    for row in rows:
        ...
        family = row.get("family_metrics_json") or {}
        for key in metric_keys:
            if key not in family:
                continue
            metrics.append(f"{key}={_format_metric(family[key])}")
...
def build_review_pack(
    *,
    ...
    rows = _load_rows(valid_db, split="valid", run_id=valid_run_id, patterns=patterns)
    rows.extend(_load_rows(calib_db, split="calib", run_id=calib_run_id, patterns=patterns))
    rows = _sort_rows(rows)

    limited: List[Dict[str, Any]] = []
    seen: Counter[tuple[str, str]] = Counter()
    for row in rows:
        key = (str(row["split"]), str(row["pattern_name"]))
        if seen[key] >= int(limit_per_pattern_split):
            continue
        seen[key] += 1
        limited.append(row)
    rows = limited
    ...
    for row in rows:
        df = _load_symbol_ohlcv(str(price_db), str(row["symbol"]))
        if df.empty:
            continue
        ...
        if window_df.empty:
            continue
        local_pivots = [int(i) - int(offset) for i in row.get("pivot_indices") or []]
        safe_id = base64.urlsafe_b64encode(str(row["pattern_id"]).encode("utf-8")).decode("ascii").rstrip("=")
        out_png = figures_dir / f"{row['split']}_{row['pattern_name']}_{safe_id}.png"
        plot_title = (
            f"{row['pattern_name']} | {row['symbol']} | conf={row.get('confidence_score')} | "
            f"w={row.get('pattern_width_bars')} | touch={row.get('touch_count')} | "
            f"h={float(row.get('pattern_height_pct') or 0.0):.1f}%"
        )
        _plot_candles(
            window_df,
            formation_start=fs,
            ...
            pivot_local_indices=local_pivots,
            title=plot_title,
            out_png=str(out_png),
        )
        row = dict(row)
    ...
    summary = {
        ...
        "by_split": dict(Counter(str(r["split"]) for r in rendered)),
        "by_pattern": dict(Counter(str(r["pattern_name"]) for r in rendered)),
        "with_breakout": sum(1 for r in rendered if r.get("breakout_date")),
        "valid_run_id": valid_run_id,
        "calib_run_id": calib_run_id,
    }

    out_dir.mkdir(parents=True, exist_ok=True)
    _write_json(str(out_dir / "review_queue.json"), rendered)
    _write_json(str(out_dir / "review_summary.json"), summary)
    _write_text(
        str(out_dir / "review_gallery.md"),
        _render_markdown(rendered, summary, out_dir, title=title, patterns=patterns, metric_keys=metric_keys),
    )
...
def main() -> None:
    ...
    parser.add_argument("--limit-per-pattern-split", type=int, default=8)
    parser.add_argument("--pre-bars", type=int, default=35)
    parser.add_argument("--post-bars", type=int, default=35)
    ...
    build_review_pack(
        ...
        limit_per_pattern_split=int(args.limit_per_pattern_split),
        pre_bars=int(args.pre_bars),
        post_bars=int(args.post_bars),
        metric_keys=_parse_metric_keys(args.metric_keys),
    )
